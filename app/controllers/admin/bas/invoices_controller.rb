module Admin
  module Bas
    class InvoicesController < Admin::BaseController
      ...
      before_action :set_invoice, only: [ :show, :ignore, :mark_needs_review, :restore ]
      before_action :block_locked_job, only: [ :ignore, :mark_needs_review, :restore ]

      def index
        @import_runs = @job.import_runs.recent
        @invoices = filtered_scope.limit(250)
      end

      def show
        @matches = @invoice.matches.includes(items: :matchable).recent
        @queries = @job.queries.where(source_type: "BasInvoice", source_id: @invoice.id).recent
      end

      def ignore
        update_record_status(@invoice, "ignored", "bas_item_ignored")
        redirect_to admin_bas_job_invoices_path(@job), notice: "Invoice ignored."
      end

      def mark_needs_review
        update_record_status(@invoice, "needs_review", "bas_item_needs_review")
        redirect_to admin_bas_job_invoices_path(@job), notice: "Invoice marked for review."
      end

      def restore
        update_record_status(@invoice, "imported", "bas_item_restored")
        redirect_to admin_bas_job_invoices_path(@job), notice: "Invoice restored."
      end
      ...
      def filtered_scope
        scope = @job.invoices.includes(:bas_import_run).recent
        scope = scope.where(status: params[:status]) if params[:status].to_s.in?(BasInvoice::STATUS_VALUES)
        ...
        if params[:q].present?
          ...
          scope = scope.where("party_name LIKE ? OR description LIKE ? OR invoice_number LIKE ?", query, query, query)
        end
      ...
      def block_locked_job
        ...
        redirect_to admin_bas_job_invoices_path(@job), alert: "Locked BAS jobs cannot have invoices changed."
      end
    end
  end
end
